...
public:
  ...
  bool dir;//死亡标志
  void Parse(FDCAN_RxHeaderTypeDef RxHeader,uint8_t RxHeaderData[]);//解析
  bool GetDir();//断连
};

inline void RM_CHxxxGy::Parse(FDCAN_RxHeaderTypeDef RxHeader, uint8_t RxHeaderData[])
{
  if(!(RxHeader.Identifier == 0x188) & !(RxHeader.Identifier == 0x288) & !(RxHeader.Identifier == 0x388))return;
  if(RxHeader.Identifier == 0x188)
	{
