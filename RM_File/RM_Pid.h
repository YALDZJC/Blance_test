...
public:
	...
	//积分隔离
	void SetMixI_Break(double maxi, double I_break);
	//分频设置
...
inline void RM_PID::SetMixI_Break(double maxi, double I_break)
{
  this->pid.MixI = maxi;
	this->pid.I_break = I_break;
}
...
//	}
	...
	if(this->pid.i < -this->pid.MixI) this->pid.i = -this->pid.MixI;
	//积分分离
	if (fabs(pid.now_e) > this->pid.I_break)
	{
		this->pid.i = 0; // 清空积分项
	}
