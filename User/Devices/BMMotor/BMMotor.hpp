...
template <uint8_t N> class BMMotorBase : public MotorBase<N>
{
  ...
  public:
     ...
     */
    void Parse(const FDCAN_RxHeaderTypeDef RxHeader, const uint8_t *pData)
    {
        const uint16_t received_id = RxHeader.Identifier;
    ...
    {
        *(uint64_t *)(&msd[0]) = 0x0202020202020202;

        RM_FDorCAN_Send(hcan, 0x38, msd); // 发送
    }
    ...
    {
        *(uint64_t *)(&msd[0]) = 0x0101010101010101;
